...
void terbanyak(list_tag &L) {
    ...
    cout<<"list yang terhubung"<<endl;
    while (next(counter) != NULL) {
        if (count(counter) != 0) {
            cout<<"tag "<<info(counter)<<" terhubung sebanyak ; "<<count(counter)<<endl;;
            if (i < count(counter)) {
                i= count(counter);
                prec = counter;
            ...
            cout<<"tag "<<info(counter)<<" tidak terhubung pad artikel manapun"<<endl;
        }
        counter = next(counter);

        }
        cout<<"tag yang paling banyak digunakan adalah tag "<<info(prec)<<endl;
        cout<<"sebanyak "<<i<<endl;
    }
